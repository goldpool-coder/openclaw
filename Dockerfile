...
RUN apt-get update && \
    ...
    npm install -g openclaw@2026.4.14 opencode-ai@latest clawhub playwright playwright-extra puppeteer-extra-plugin-stealth @steipete/bird && \
    # 安装 bun、uv 和 qmd
...
RUN if [ -n "$CLAWHUB_TOKEN" ]; then clawhub login --token "$CLAWHUB_TOKEN"; fi && \
  ...
  printf '%s\n' '2026.4.14' > /home/node/.openclaw-seed/extensions/.seed-version && \
