...
<<<<<<< HEAD
    ...
    npm install -g openclaw@2026.3.23-2 opencode-ai@latest playwright playwright-extra puppeteer-extra-plugin-stealth @steipete/bird clawhub && \
=======
    # 设置 npm 镜像并安装全局包
...
# 进入 Homebrew 的核心仓库目录，强制删除可能损坏的 .git 目录，然后重新初始化
# 这样可以确保 brew update 能正常工作
RUN brew update && \
    brew install gh && \
